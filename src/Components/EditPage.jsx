    ...
    <main className="min-h-[calc(100vh-73px)] bg-[#0b3534] px-4 py-8 sm:px-8 lg:py-12">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_310px]">
          <section aria-labelledby="preview-heading" className="rounded-3xl border border-white/10 bg-[#082b2b] p-3 shadow-2xl shadow-black/15 sm:p-5">
            <div className="mb-4 flex items-center justify-between px-1 text-xs font-medium text-slate-400"><h2 id="preview-heading" className="flex items-center gap-2 font-medium"><FiImage className="text-lime-300" /> Live preview</h2><span>{width || "–"} × {height || "–"} px</span></div>
          ...
          <aside aria-labelledby="output-size-heading" className="rounded-3xl border border-white/10 bg-[#f5f7f3] p-5 shadow-xl sm:p-6">
            <div className="flex items-center gap-3 border-b border-slate-200 pb-5"><span className="grid size-10 place-items-center rounded-xl bg-[#0b3534] text-lime-300"><FiMaximize2 /></span><div><h2 id="output-size-heading" className="font-bold text-slate-900">Output size</h2><p className="text-xs text-slate-500">Set your exact dimensions</p></div></div>
            ...
            <section aria-labelledby="quick-presets-heading" className="mt-6"><h3 id="quick-presets-heading" className="text-xs font-semibold uppercase tracking-wide text-slate-500">Quick presets</h3><div className="mt-3 grid grid-cols-2 gap-2">{[[1080, 1080, "Square"], [1080, 1350, "Portrait"], [1200, 630, "Landscape"], [1920, 1080, "HD"]].map(([presetWidth, presetHeight, name]) => <button key={name} onClick={() => applyPreset(presetWidth, presetHeight)} className="rounded-xl border border-slate-200 bg-white px-2 py-2.5 text-xs font-semibold text-slate-600 transition hover:border-teal-600 hover:bg-teal-50 hover:text-teal-800">{name}<span className="ml-1 text-slate-400">{presetWidth}×{presetHeight}</span></button>)}</div></section>
            <section aria-labelledby="target-file-size-heading" className="mt-6 border-t border-slate-200 pt-6"><h3 id="target-file-size-heading" className="text-xs font-semibold uppercase tracking-wide text-slate-500">Target file size (KB)</h3><label className="sr-only" htmlFor="target-file-size">Target file size in KB</label><input id="target-file-size" type="number" min="1" value={targetSize} onChange={(event) => setTargetSize(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm font-bold text-slate-800 outline-none transition focus:border-teal-600 focus:ring-3 focus:ring-teal-600/15" /><p className="mt-2 text-xs leading-5 text-slate-500">Downloads as JPG and keeps the file at or below this KB target.</p></section>
